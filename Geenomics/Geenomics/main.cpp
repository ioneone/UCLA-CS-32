...
//  Geenomics
//
//  Created by Junhong Wang on 3/9/19.
...
//    trie.insert("hat", 9);
    ...
    std::vector<int> result0 = trie.find("sit", false);
    std::cout << "=====" << std::endl;
    for (int i : result0) {
        std::cout << i << ", ";
    }
    ...
    std::vector<int> result2 = trie.find("hit", false);
    for (int i : result2) {
        std::cout << i << ", ";
    }
    std::cout << std::endl;
    
    std::vector<int> result3 = trie.find("sit", false);
    for (int i : result3) {
        std::cout << i << ", ";
    }
    std::cout << std::endl;
    ...
    std::vector<Genome> vg;
    bool success = Genome::load(strm, vg);
    
    if (success) {
        std::cout << "Loaded " << vg.size() << " genomes successfully:" << std::endl;
        for (int k = 0; k != vg.size(); k++)
            std::cout << vg[k].name() << std::endl;
    } else {
        std::cout << "Error loading genome data" << std::endl;
    }
    
    GenomeMatcher matcher(4);
    for (Genome genome : vg) {
        matcher.addGenome(genome);
    }
    ...
    if (result) {
        for (DNAMatch match : matches) {
            std::cout << match.genomeName << " of length " << match.length << " at position " << match.position << std::endl;
        }
    }
...
int myTest2() {
    GenomeMatcher matcher(4);
    Genome g("sasquatch", "GGGGTTTTAAAACCCCACGTACGTACGTNANANANA");
    matcher.addGenome(g);
    
    std::vector<GenomeMatch> results;
    Genome genome("test", "AAATCCCTGGGGTTTTNANA");
    
    if (matcher.findRelatedGenomes(genome, 2 * matcher.minimumSearchLength(), false, 20, results)) {
        std::cout << "found!" << std::endl;
        for (GenomeMatch match : results) {
            std::cout << match.genomeName << ": " << match.percentMatch << std::endl;
        }
    } else {
        std::cout << "No related genomes found" << std::endl;
    }
...
//        }
//    } else {
//        cout << "No related genomes found" << endl;
//    }
//
//    return 0;
